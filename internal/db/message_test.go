...
func TestInsertMessage(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			message:  "Hello, World!",
			mockBehavior: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec("INSERT INTO chat_message \\(user_id, username, message\\) VALUES \\(\\?, \\?, \\?\\);").
	...
	tests := []struct {
	...
	}{
		{
			...
			mockBehavior: func(mock sqlmock.Sqlmock) {
				...
				mock.ExpectQuery("SELECT id, user_id, username, message, created_at FROM chat_message;").
					WillReturnRows(rows)
			},
			expected: []*pb.Message{
				{
		...
		},
		{
			...
			mockBehavior: func(mock sqlmock.Sqlmock) {
				rows := sqlmock.NewRows([]string{"id", "user_id", "username", "message", "created_at"}).
					AddRow(1, 1, "testuser", "Hello, World!", "2024-08-15 00:00:00")
				mock.ExpectQuery("SELECT id, user_id, username, message FROM chat_message;").
					WillReturnRows(rows)
			},
			expected:  nil,
			expectErr: true,
